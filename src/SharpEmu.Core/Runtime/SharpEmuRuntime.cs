...
public sealed class SharpEmuRuntime : ISharpEmuRuntime
{
    ...
    public static ISharpEmuRuntime CreateDefault(SharpEmuRuntimeOptions options = default)
    {
        ...
        var moduleManager = HleModuleCatalog.CreateRegisteredModuleManager();
    ...
    public OrbisGen2Result Run(string ebootPath)
    {
        ...
        {
            Log.Info($"Initializer dispatch failed: {failedInitializerResult}");
        ...
        Log.Info($"About to call DispatchEntry with entryPoint=0x{image.EntryPoint:X16}");
        ...
        IDictionary<string, ulong> runtimeSymbols)
    {
        ...
        Log.Info($"Module search directories: {string.Join(", ", moduleDirectories)}");
        ...
        foreach (var modulePath in modulePaths)
        {
            ...
            {
                failedModules++;
                Log.Info($"Module load failed: {modulePath} ({ex.GetType().Name}: {ex.Message})");
            }
        }
